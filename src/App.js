...
import { 
  Plus, Trash2, Leaf, Sun, Moon, Search, Zap, Award, Loader2, X 
} from 'lucide-react';
  ...
  const getAiSuggestions = async () => {
    ...
    setTimeout(() => {
      ...
      if (content.includes("eat") || content.includes("food")) {
        ...
        const randomTask = ECO_TASKS[Math.floor(Math.random() * ECO_TASKS.length)];
        suggestion = { ecoAction: randomTask };
      }
  ...
  const toggleEcoTask = (noteId, task) => {
    setNotes(notes.map(note => {
      if (note.id === noteId) {
        ...
        const updatedNote = { ...note, completedEcoTasks: newCompleted, points: newCompleted.length * 10 };

        if (activeNote?.id === noteId) setActiveNote(updatedNote);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredNotes.map(note => (
            <div
              ...
              className={`p-4 rounded-2xl border cursor-pointer hover:shadow-lg transition ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100 shadow-sm'}`}
            >
              ...
              {note.ecoTasks.length > 0 && (
                <div className="border-t dark:border-slate-700 pt-3 flex flex-wrap gap-2">
                  {note.ecoTasks.map(t => (
                    <button
                      key={t}
                      onClick={(e) => { e.stopPropagation(); toggleEcoTask(note.id, t); }}
                      className={`text-[10px] px-2 py-1 rounded-md border ${note.completedEcoTasks.includes(t) ? 'bg-emerald-500 text-white' : 'text-slate-500'}`}
                    >
                      {t}
                    </button>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className={`w-full max-w-lg p-6 rounded-3xl ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">{activeNote.title || 'Untitled'}</h2>
              <button onClick={() => setActiveNote(null)}><X /></button>
            </div>
            <p className="mb-4">{activeNote.content}</p>
            {activeNote.ecoTasks.length > 0 && (
              <div className="border-t dark:border-slate-700 pt-3 flex flex-wrap gap-2">
                {activeNote.ecoTasks.map(t => (
                  <button
                    key={t}
                    onClick={() => toggleEcoTask(activeNote.id, t)}
                    className={`text-[10px] px-2 py-1 rounded-md border ${activeNote.completedEcoTasks.includes(t) ? 'bg-emerald-500 text-white' : 'text-slate-500'}`}
                  >
                    {t}
                  </button>
                ))}
            ...
            <p className="mt-2 text-xs text-slate-400">{activeNote.points} pts</p>
          </div>
        </div>
      )}
